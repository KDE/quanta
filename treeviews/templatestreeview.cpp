    ...
    if (item)
    {
      ...
      if ( !url.isEmpty() )
      {
        ...
        *(tempFile->textStream()) << content;
        ...
        {
          proceed = KMessageBox::warningContinueCancel(this, i18n("<qt>The file <b>%1</b> already exists.<br>Do you want to overwrite it?</qt>", url.prettyURL(0, KURL::StripFileProtocol)),i18n("Overwrite"), i18n("Overwrite")) == KMessageBox::Continue;
        }
        if (proceed)
        {
          if (!QuantaNetAccess::upload(tempFile->name(), url, m_parent, false))
          {
            KMessageBox::error(this,i18n("<qt>Could not write to file <b>%1</b>.<br>Check if you have rights to write there or that your connection is working.</qt>", url.prettyURL(0, KURL::StripFileProtocol)));
          }
        }
        delete tempFile;
      }
...
void TemplatesTreeView::slotExtractSiteTemplate()
{
   ...
   if (urlRequester.exec())
   {
      ...
      if (KIO::NetAccess::download(url, tempFile, this))
      {
         ...
         error = true;
      if (error)
        KMessageBox::error(this, i18n("<qt>Some error happened while extracting the <i>%1</i> site template file.<br>Check that you have write permission for <i>%2</i> and that there is enough free space in your temporary folder.</qt>", url.prettyURL(0, KURL::StripFileProtocol), targetURL.prettyURL(0, KURL::StripFileProtocol)));
   }
}
