...
void EventConfigurationDlg::slotDeleteEvent()
{
   QListViewItem *item = eventsListView->currentItem();
   if (!item) return;
   if (KMessageBox::warningYesNo(this, i18n("<qt>Are you sure that you want to remove the configuration of the <b>%1</b> event?</qt>").arg(item->text(0)), i18n("Delete Event Configuration")) == KMessageBox::Yes)
   {
      delete item;
   }
}
