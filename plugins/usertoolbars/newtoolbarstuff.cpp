...
{
    KUrl destURL = KUrl(KGlobal::dirs()->saveLocation("data") + "kdevusertoolbars/toolbars/" + QFileInfo(m_tarName).fileName());
    bool ok = true;
    if (ExtFileInfo::checkOverwrite(destURL))
    {
        if (!ExtFileInfo::copy(KUrl(m_tarName), destURL, -1, true, false, parentWidget()))
          ok = false;
        else
        {
