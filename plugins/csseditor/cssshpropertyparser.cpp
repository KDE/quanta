...
{
  QStringList l1,l2=QStringList::split(",",s);
...
QString CSSSHPropertyParser::removeEndingWhiteSpaces(QString s)
{
...
QString CSSSHPropertyParser::removeBeginningWhiteSpaces(QString s)
{
   int index = 0;
   while(s[index] == ' ' )
     index++;
...
QString CSSSHPropertyParser::extractQuotedStringList()
{
   ...
   while(!stop && i<m_propertyToParse.length() ){
      if( m_propertyToParse[i] == ' ' ){
        if( ( temp.contains("\"") + temp.contains("\'") )%2 == 0 )
          stop = true;
        else   
