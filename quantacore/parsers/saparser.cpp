...
void SAParser::init(ParseResult *base, EditorSource *source, const DTDStruct *dtd)
{
  m_base = base;
  m_source = source;
  m_dtd = dtd;
  ...
  if (s_line <= s_endLine)
  {
    ...
    switch (s_currentContext.type)
    {
      ...
      case Text: {
        ...
        if (s_searchContent || (s_parentNode && s_parentNode->tag->dtd()->family == DTDStruct::Xml))
        {
          ...
          {
            quotedStringPos = s_textLine.indexOf(m_quotesRx, s_col); //quoted strings
            s_searchedString = s_textLine.left(quotedStringPos);
            commentPos = s_searchedString.indexOf(s_dtd->commentsStartRx, s_col); //comments
            s_searchedString = s_textLine.left(commentPos);
            if (s_fullParse)
              groupKeywordPos = s_searchedString.indexOf(s_dtd->structRx, s_col); //groups, like { }
          } else
...
//               s_context.area.bCol = quotedStringPos;
              s_context.area.start.setY(quotedStringPos);
          ...
          if (!s_contextFound && commentPos != -1)  //is it a comment?
          {
                ...
                (commentPos < areaEndPos || areaEndPos == -1) )
            {
              ...
              s_context.area.start.setX(commentPos);
...
//                 s_context.area.bCol = groupKeywordPos;
                s_context.area.start.ry() = groupKeywordPos;
...
/*                s_currentContext.area.eLine = s_line;
                ...
                s_currentContext.area.setEnd(s_line, groupKeywordPos + foundText.length() - 1);
                if (s_currentNode &&
                    (s_currentNode->tag->isType(Tag::Text) ||
                     s_currentNode->tag->isType(Tag::Empty)) )
                {
                  QString areaString = m_source->text(s_currentContext.area);
                  ParserCommon::appendAreaToTextNode(areaString, s_currentContext.area, s_currentNode);
                }
                else
                  s_currentNode = ParserCommon::createTextNode(m_source, m_dtd, s_currentNode, s_line, s_currentContext.area.eCol() + 1, s_currentContext.parentNode);

                s_currentNode->tag->setType(Tag::ScriptStructureBegin);
...
/*                s_currentContext.area.bLine = s_line;
                ...
                s_currentContext.area.setStart(s_line, s_col);
              ...
              } else  //it's a closing group structure element (like "}")
              {
                     ...
                     s_currentNode->tag->isType(Tag::Empty)) )
                {
                  ...
                  s_currentNode = ParserCommon::createTextNode(m_source, m_dtd, s_currentNode, s_line, groupKeywordPos, s_currentContext.parentNode);
                if (s_currentNode)
                {
                  s_currentNode->insideSpecial = true;
                  s_currentNode->specialInsideXml = m_specialInsideXml;
...
/*              s_currentContext.area.eLine = s_line;
              ...
              s_currentContext.area.setEnd(s_line, specialAreaPos - 1);
              if (s_fullParse)
              {
                if ( s_currentNode &&
                    (s_currentNode->tag->isType(Tag::Text) ||
                    s_currentNode->tag->isType(Tag::Empty)) )
                {
                  ...
                  s_currentNode = ParserCommon::createTextNode(m_source, m_dtd, s_currentNode, s_line, specialAreaPos, s_currentContext.parentNode);
                if (s_currentNode)
                {
                  s_currentNode->insideSpecial = true;
                  s_currentNode->specialInsideXml = m_specialInsideXml;
...
/*              s_currentContext.area.bLine = s_line;
              ...
              s_currentContext.area.setStart(s_line, s_col + 1);
              s_textLine = m_source->getLine(s_line, s_endLine, s_endCol);
              s_col++;
              if (m_synchronous)
              {
                return true;
        ...
        {
          areaEndPos = s_textLine.indexOf(s_areaEndString, s_col);
        } else
        if (s_searchForForcedAreaEnd)
        {
          areaEndPos = s_textLine.indexOf(s_forcedAreaRx, s_col);
          if (areaEndPos != -1)
            s_areaEndString = s_forcedAreaRx.cap();
        }
...
/*          s_currentContext.area.eLine = s_line;
          ...
          s_currentContext.area.setEnd(s_line, areaEndPos - 1);
          ...
          if (s_fullParse || !s_parentNode->child)
          {
            ...
            {
              s_currentNode = ParserCommon::createTextNode(m_source, m_dtd, s_currentNode, s_line, areaEndPos, s_parentNode);
            }
            if (s_currentNode)
            {
              s_currentNode->insideSpecial = true;
  ...
  //FIXME          nodeNum++;
            if (s_parentNode->next)
            {
              node->next = s_parentNode->next;
              s_parentNode->next->prev = node;
            }
...
//            slotParseForScriptGroup();
          ...
          m_lastParsedNode = s_parentNode->next;
        ...
        if (s_contextFound)
        {
          s_context.area = AreaStruct(s_line, s_context.area.bCol(), -1, -1);
          s_context.parentNode = s_currentContext.parentNode;
          s_currentContext.area.end = s_context.area.start;
          s_currentContext.area.end.ry()--;
       //   s_currentContext.parentNode = s_parentNode;
          s_contextStack.push(s_currentContext);
          if (s_fullParse)
          {
            if (s_currentNode &&
              (s_currentNode->tag->isType(Tag::Text) || s_currentNode->tag->isType(Tag::Empty)) )
            {
              QString areaString = m_source->text(s_currentContext.area);
              ParserCommon::appendAreaToTextNode(areaString, s_currentContext.area, s_currentNode);
              s_currentNode->insideSpecial = true;
            ...
            if (s_currentContext.area.notEmpty())
            {
              //create a tag from the s_currentContext
              QString tagStr = m_source->text(s_currentContext.area);
              Tag *tag = new Tag(s_currentContext.area, tagStr, s_dtd);
              tag->cleanStr = tagStr;
              QuantaCommon::removeCommentsAndQuotes(tag->cleanStr, s_dtd);
              if (tagStr.simplified().isEmpty())
              {
                tag->setType(Tag::Empty);
              } else
              {
                tag->setType(Tag::Text);
              }
              tag->single = true;
     ...
     case QuotedString:
      {
        ...
        while (p < l)
        {
          p = s_textLine.indexOf(s_currentContext.startString, p);
          if (p != -1)
          {
...
/*          s_currentContext.area.eLine = s_line;
          ...
          if (s_fullParse)
          {
                ...
                s_currentNode->tag->isType(Tag::Empty)) )
            {
              ...
              s_currentNode = ParserCommon::createTextNode(m_source, m_dtd, 0L, s_line, pos, s_currentContext.parentNode);
            s_currentNode->insideSpecial = true;
            s_currentNode->specialInsideXml = m_specialInsideXml;
          }
          s_previousContext = s_contextStack.pop();
...
/*          s_currentContext.area.bLine = s_line;
          ...
          s_currentContext.area = AreaStruct(s_line, pos + 1, -1, -1);
          s_currentContext.startString = s_previousContext.startString;
          s_col = pos + 1;
        } else
        {
     ...
     case Comment:
      {
        int pos = s_textLine.indexOf(s_dtd->comments[s_currentContext.startString], s_col);
        if (pos == -1 && s_dtd->comments[s_currentContext.startString] == "\n")
        {
          int pos2 = s_textLine.indexOf(s_areaEndString, s_col);
          if (pos2 != -1)
          {
            pos = pos2 - 1;
          } else
...
/*          s_currentContext.area.eLine = s_line;
          ...
          if (s_fullParse)
          {
            ...
            QString areaString = m_source->text(s_currentContext.area);
            Tag *tag = new Tag(s_currentContext.area, areaString, s_dtd);
            tag->setType(Tag::Comment);
            tag->single = true;
            //create a node with the above tag
            Node *node = new Node(s_currentContext.parentNode);
//FIXME            nodeNum++;
...
/*          s_currentContext.area.bLine = s_line;
          ...
          s_currentContext.area.eLine = s_currentContext.area.eCol = -1;*/
          s_currentContext.area = AreaStruct(s_line, s_currentContext.area.eCol() + 1, -1, -1);
          s_currentContext.startString = s_previousContext.startString;
          s_col = s_currentContext.area.bCol();
        } else
        {
...
/*  s_currentContext.area.bLine = s_line;
  s_currentContext.area.bCol = s_col;
  s_currentContext.area.eLine = s_currentContext.area.eCol = -1;*/
  s_currentContext.area = AreaStruct(s_line, s_col, -1, -1);
  s_currentContext.parentNode = s_parentNode;
...
Node *SAParser::parsingDone()
{
  ...
  if (!s_currentNode)
  {
    s_currentNode = ParserCommon::createTextNode(m_source, m_dtd, s_parentNode, s_endLine, s_endCol, s_parentNode);
    if (s_currentNode)
    {
      s_currentNode->insideSpecial = true;
      s_currentNode->specialInsideXml = m_specialInsideXml;
  ...
  {
    s_currentNode = ParserCommon::createTextNode(m_source, m_dtd, s_currentNode, s_endLine, s_endCol, s_parentNode);
    s_currentNode->insideSpecial = true;
    s_currentNode->specialInsideXml = m_specialInsideXml;
  }
  if (s_fullParse)
...
void SAParser::slotParseNodeInDetail()
{
  ...
  if (m_currentNode && m_parsingEnabled && m_base->baseNode)
  {
    if (m_currentNode->insideSpecial &&
        !m_currentNode->tag->isType(Tag::Comment) &&
        !m_currentNode->tag->isType(Tag::Text) &&
        !m_currentNode->tag->isType(Tag::Empty))
    {
      ...
      {
        area.start = m_currentNode->tag->area().end;
        ++(area.start.ry());
...
{
  if (m_base->dtepList.contains(dtd) == 0)
  {
    m_base->dtepList.append(dtd);
  }
}
