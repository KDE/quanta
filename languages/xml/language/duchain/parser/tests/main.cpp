...
class SgmlParser {
    public:
        SgmlParser(const bool debug, const bool printTokens)
    ...
    private:
        ...
        */
        ...
        {
            if (m_printTokens) {
                Xml::TokenStream tokenStream;
                SgmlTokenizer lexer(&tokenStream, m_session.contents().toLatin1().data());
                int token;
            ...
            if (!m_session.parse(&ast)) {
                ...
                if (m_debug) {
                    Xml::DebugVisitor debugVisitor(m_session.tokenStream(), m_session.contents());
        ...
        void printToken(int token, const SgmlTokenizer& lexer) const
        {
...
int main(int argc, char* argv[])
{
    ...
    SgmlParser parser(debug, printTokens);
...
void showUsage(char** argv)
{
    ...
    qout << "--code ...     All following arguments will be interpreted as XML code." << endl
